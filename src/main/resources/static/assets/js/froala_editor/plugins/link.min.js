...
!function (e, t) {
    ...
    y = y && y.hasOwnProperty("default") ? y["default"] : y, Object.assign(y.POPUP_TEMPLATES, {
        ...
        linkText: !0
    }), y.PLUGINS.link = function (d) {
        var u = d.$;

        function k() {
            var e = d.image ? d.image.get() : null;
            if (e || !d.$wp) return "A" == d.el.tagName ? d.el : e && e.get(0).parentNode && "A" == e.get(0).parentNode.tagName ? e.get(0).parentNode : void 0;
            ...
            var n = d.selection.element(), i = d.selection.endElement();
            "A" == n.tagName || d.node.isElement(n) || (n = u(n).parentsUntil(d.$el, "a").first().get(0)), "A" == i.tagName || d.node.isElement(i) || (i = u(i).parentsUntil(d.$el, "a").first().get(0));
            try {
        ...
        function g() {
            var e, t, n, i, r = d.image ? d.image.get() : null, l = [];
        ...
        function h(r) {
            if (d.core.hasFocus()) {
                if (l(), r && "keyup" === r.type && (r.altKey || r.which == y.KEYCODE.ALT)) return !0;
                setTimeout(function () {
                    if (!r || r && (1 == r.which || "mouseup" != r.type)) {
                        var e = k(), t = d.image ? d.image.get() : null;
                        if (e && !t) {
                            if (d.image) {
                                var n = d.node.contents(e);
                                if (1 == n.length && "IMG" == n[0].tagName) {
                                    var i = d.selection.ranges(0);
                                    return 0 === i.startOffset && 0 === i.endOffset ? u(e).before(y.MARKERS) : u(e).after(y.MARKERS), d.selection.restore(), !1
                                }
                            }
                            r && r.stopPropagation(), a(e)
                        }
                    }
        ...
        function a(e) {
            ...
            t || (t = function l() {
                var e = "";
                1 <= d.opts.linkEditButtons.length && ("A" == d.el.tagName && 0 <= d.opts.linkEditButtons.indexOf("linkRemove") && d.opts.linkEditButtons.splice(d.opts.linkEditButtons.indexOf("linkRemove"), 1), e = '<div class="fr-buttons">'.concat(d.button.buildList(d.opts.linkEditButtons), "</div>"));
                var t = {buttons: e}, n = d.popups.create("link.edit", t);
                d.$wp && d.events.$on(d.$wp, "scroll.link-edit", function () {
                    k() && d.popups.isVisible("link.edit") && a(k())
                });
                return n
            }());
            var n = u(e);
            ...
            d.popups.show("link.edit", i, r, n.outerHeight(), !0)
        }
        ...
        function o() {
            var e = d.popups.get("link.insert"), t = k();
            if (t) {
                var n, i, r = u(t), l = e.find('input.fr-link-attr[type="text"]'),
                    a = e.find('input.fr-link-attr[type="checkbox"]');
                for (n = 0; n < l.length; n++) (i = u(l[n])).val(r.attr(i.attr("name") || ""));
                for (a.attr("checked", !1), n = 0; n < a.length; n++) i = u(a[n]), r.attr(i.attr("name")) == i.data("checked") && i.attr("checked", !0);
                e.find('input.fr-link-attr[type="text"][name="text"]').val(r.text())
            } else e.find('input.fr-link-attr[type="text"]').val(""), e.find('input.fr-link-attr[type="checkbox"]').attr("checked", !1), e.find('input.fr-link-attr[type="text"][name="text"]').val(d.selection.text());
        ...
        function s(e) {
            if (e) return d.popups.onRefresh("link.insert", o), !0;
            var t = "";
            1 <= d.opts.linkInsertButtons.length && (t = '<div class="fr-buttons fr-tabs">' + d.button.buildList(d.opts.linkInsertButtons) + "</div>");
            var n = "", i = 0;
            ...
            d.opts.linkAlwaysBlank || (n += '<div class="fr-checkbox-line"><span class="fr-checkbox"><input name="target" class="fr-link-attr" data-checked="_blank" type="checkbox" id="fr-link-target-'.concat(d.id, '" tabIndex="').concat(++i, '"><span>').concat('<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="10" height="10" viewBox="0 0 32 32"><path d="M27 4l-15 15-7-7-5 5 12 12 20-20z" fill="#FFF"></path></svg>', '</span></span><label id="fr-label-target-').concat(d.id, '">').concat(d.language.translate("Open in new tab"), "</label></div>"));
            var a = {
                buttons: t,
            ...
            return d.$wp && d.events.$on(d.$wp, "scroll.link-insert", function () {
                (d.image ? d.image.get() : null) && d.popups.isVisible("link.insert") && c(), d.popups.isVisible("link.insert") && f()
            }), s
        }
        ...
        function p(e, t, n) {
            ...
            if ("" === d.opts.linkAutoPrefix || new RegExp("^(" + y.LinkProtocols.join("|") + "):.", "i").test(e) || /^data:image.*/i.test(e) || /^(https?:|ftps?:|file:|)\/\//i.test(e) || /^([A-Za-z]:(\\){1,2}|[A-Za-z]:((\\){1,2}[^\\]+)+)(\\)?$/i.test(e) || ["/", "{", "[", "#", "(", "."].indexOf((e || "")[0]) < 0 && (e = d.opts.linkAutoPrefix + d.helpers.sanitizeURL(e)), e = d.helpers.sanitizeURL(e), d.opts.linkAlwaysBlank && (n.target = "_blank"), d.opts.linkAlwaysNoFollow && (n.rel = "nofollow"), d.helpers.isEmail(r) && (n.target = null, n.rel = null), "_blank" == n.target ? (d.opts.linkNoOpener && (n.rel ? n.rel += " noopener" : n.rel = "noopener"), d.opts.linkNoReferrer && (n.rel ? n.rel += " noreferrer" : n.rel = "noreferrer")) : null == n.target && (n.rel ? n.rel = n.rel.replace(/noopener/, "").replace(/noreferrer/, "") : n.rel = null), t = t || "", e === d.opts.linkAutoPrefix) return d.popups.get("link.insert").find('input[name="href"]').addClass("fr-error"), d.events.trigger("link.bad", [r]), !1;
            var l, a = k();
            if (a) {
                if ((l = u(a)).attr("href", e), 0 < t.length && l.text() != t && !i) {
                    for (var s = l.get(0); 1 === s.childNodes.length && s.childNodes[0].nodeType == Node.ELEMENT_NODE;) s = s.childNodes[0];
                    u(s).text(t)
                }
                i || l.prepend(y.START_MARKER).append(y.END_MARKER), l.attr(n), i || d.selection.restore()
            } else {
                i ? (i.wrap('<a href="' + e + '"></a>'), d.image.hasCaption() && i.parent().append(i.parents(".fr-img-caption").find(".fr-inner"))) : (d.format.remove("a"), d.selection.isCollapsed() ? (t = 0 === t.length ? r : t, d.html.insert('<a href="' + e + '">' + y.START_MARKER + t.replace(/&/g, "&amp;").replace(/</, "&lt;", ">", "&gt;") + y.END_MARKER + "</a>"), d.selection.restore()) : 0 < t.length && t != d.selection.text().replace(/\n/g, "") ? (d.selection.remove(), d.html.insert('<a href="' + e + '">' + y.START_MARKER + t.replace(/&/g, "&amp;") + y.END_MARKER + "</a>"), d.selection.restore()) : (!function c() {
                ...
                for (var o = g(), p = 0; p < o.length; p++) (l = u(o[p])).attr(n), l.removeAttr("_moz_dirty");
                1 == o.length && d.$wp && !i && (u(o[0]).prepend(y.START_MARKER).append(y.END_MARKER), d.selection.restore())
            }
            if (i) {
        ...
        function f() {
            ...
            if (e) {
                ...
                var n = (d.image ? d.image.get() : null) || u(e), i = n.offset().left + n.outerWidth() / 2,
                    r = n.offset().top + n.outerHeight();
                d.popups.show("link.insert", i, r, n.outerHeight(), !0)
            }
        }
        ...
        function c() {
            ...
            if (e) {
                var t = d.popups.get("link.insert");
                d.image.hasCaption() && (e = e.find(".fr-img-wrap")), t || (t = s()), o(), d.popups.setContainer("link.insert", d.$sc);
                var n = e.offset().left + e.outerWidth() / 2, i = e.offset().top + e.outerHeight();
                d.popups.show("link.insert", n, i, e.outerHeight(), !0)
            }
        }
        ...
        return {
            _init: function e() {
                d.events.on("keyup", function (e) {
                    e.which != y.KEYCODE.ESC && h(e)
                }), d.events.on("window.mouseup", h), d.events.$on(d.$el, "click", "a", function (e) {
                    d.edit.isDisabled() && e.preventDefault()
                }), d.helpers.isMobile() && d.events.$on(d.$doc, "selectionchange", h), s(!0), "A" == d.el.tagName && d.$el.addClass("fr-view"), d.events.on("toolbar.esc", function () {
                ...
                t && e ? (t.unwrap(), d.image.edit(t)) : e && (d.selection.save(), u(e).replaceWith(u(e).html()), d.selection.restore(), l())
            }, showInsertPopup: function m() {
                var e = d.$tb.find('.fr-command[data-cmd="insertLink"]'), t = d.popups.get("link.insert");
                if (t || (t = s()), !t.hasClass("fr-active")) if (d.popups.refresh("link.insert"), d.popups.setContainer("link.insert", d.$tb || d.$sc), e.isVisible()) {
                    var n = d.button.getPosition(e), i = n.left, r = n.top;
                    d.popups.show("link.insert", i, r, e.outerHeight())
                } else d.position.forSelection(t), d.popups.show("link.insert")
            }, usePredefined: function v(e) {
                var t, n, i = d.opts.linkList[e], r = d.popups.get("link.insert"),
                    l = r.find('input.fr-link-attr[type="text"]'), a = r.find('input.fr-link-attr[type="checkbox"]');
                for (n = 0; n < l.length; n++) i[(t = u(l[n])).attr("name")] ? (t.val(i[t.attr("name")]), t.toggleClass("fr-not-empty", !0)) : "text" != t.attr("name") && t.val("");
                for (n = 0; n < a.length; n++) (t = u(a[n])).attr("checked", t.data("checked") == i[t.attr("name")]);
                d.accessibility.focusPopup(r)
            }, insertCallback: function b() {
                var e, t, n = d.popups.get("link.insert"), i = n.find('input.fr-link-attr[type="text"]'),
                    r = n.find('input.fr-link-attr[type="checkbox"]'),
                    l = (i.filter('[name="href"]').val() || "").trim(), a = i.filter('[name="text"]').val(), s = {};
                for (t = 0; t < i.length; t++) e = u(i[t]), ["href", "text"].indexOf(e.attr("name")) < 0 && (s[e.attr("name")] = e.val());
                ...
                p(l, a, s), u(d.o_win).scrollTop(o)
            }, insert: p, update: f, get: k, allSelected: g, back: function t() {
                d.image && d.image.get() ? d.image.back() : (d.events.disableBlur(), d.selection.restore(), d.events.enableBlur(), k() && d.$wp ? (d.selection.restore(), l(), h()) : "A" == d.el.tagName ? (d.$el.focus(), h()) : (d.popups.hide("link.insert"), d.toolbar.showInline()))
            }, imageLink: c, applyStyle: function E(e, t, n) {
                void 0 === n && (n = d.opts.linkMultipleStyles), void 0 === t && (t = d.opts.linkStyles);
                var i = k();
                if (!i) return !1;
                if (!n) {
                    var r = Object.keys(t);
                    r.splice(r.indexOf(e), 1), u(i).removeClass(r.join(" "))
                }
                u(i).toggleClass(e), h()
            }
        }
    ...
    }), y.DefineIcon("linkBack", {NAME: "arrow-left", SVG_KEY: "back"}), y.RegisterCommand("linkBack", {
        title: "Back",
        undo: !1,
        focus: !1,
        back: !0,
