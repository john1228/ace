...
!function (e, t) {
    ...
    c = c && c.hasOwnProperty("default") ? c["default"] : c, Object.assign(c.DEFAULTS, {dragInline: !0}), c.PLUGINS.draggable = function (g) {
        ...
        function e(e) {
            return !(!e.originalEvent || !e.originalEvent.target || e.originalEvent.target.nodeType !== Node.TEXT_NODE) || (e.target && "A" === e.target.tagName && 1 === e.target.childNodes.length && "IMG" === e.target.childNodes[0].tagName && (e.target = e.target.childNodes[0]), p(e.target).hasClass("fr-draggable") ? (g.undo.canDo() || g.undo.saveStep(), g.opts.dragInline ? g.$el.attr("contenteditable", !0) : g.$el.attr("contenteditable", !1), g.opts.toolbarInline && g.toolbar.hide(), p(e.target).addClass("fr-dragging"), g.browser.msie || g.browser.edge || g.selection.clear(), void e.originalEvent.dataTransfer.setData("text", "Froala")) : (e.preventDefault(), !1))
        }
        ...
        function m(e, t, r) {
            ...
            d.offset().top !== e && d.css("top", e), d.offset().left !== t && d.css("left", t), d.width() !== r && d.css("width", r)
        }
        ...
        function t(e) {
            e.originalEvent.dataTransfer.dropEffect = "move", g.opts.dragInline ? function r() {
                ...
                if (!v(t)) {
                    ...
                    (!v(n) || d && 0 === g.$el.find(n).length && n !== d.get(0)) && (n = null);
                    for (var a = 0, o = t; !v(o) && o === t && e.originalEvent.pageY - g.win.pageYOffset + a < p(g.doc).height();) a++, o = g.doc.elementFromPoint(e.originalEvent.pageX - g.win.pageXOffset, e.originalEvent.pageY - g.win.pageYOffset + a);
                    (!v(o) || d && 0 === g.$el.find(o).length && o !== d.get(0)) && (o = null), t = null === o && n ? n : o && null === n ? o : o && n ? r < a ? n : o : null
                }
                if (p(t).hasClass("fr-drag-helper")) return !1;
                if (t && !g.node.isBlock(t) && (t = g.node.blockParent(t)), t && 0 <= ["TD", "TH", "TR", "THEAD", "TBODY"].indexOf(t.tagName) && (t = p(t).parents("table").get(0)), t && 0 <= ["LI"].indexOf(t.tagName) && (t = p(t).parents("UL, OL").get(0)), t && !p(t).hasClass("fr-drag-helper")) {
                    var i;
                    d || (c.$draggable_helper || (c.$draggable_helper = p(document.createElement("div")).attr("class", "fr-drag-helper")), d = c.$draggable_helper, g.events.on("shared.destroy", function () {
                        d.html("").removeData().remove(), d = null
                    }, !0)), i = e.originalEvent.pageY < p(t).offset().top + p(t).outerHeight() / 2;
                    var l = p(t), s = 0;
                    i || 0 !== l.next().length ? (i || (l = l.next()), "before" === d.data("fr-position") && l.is(d.data("fr-tag")) || (0 < l.prev().length && (s = parseFloat(l.prev().css("margin-bottom")) || 0), s = Math.max(s, parseFloat(l.css("margin-top")) || 0), m(l.offset().top - s / 2 - g.$box.offset().top, l.offset().left - g.win.pageXOffset - g.$box.offset().left, l.width()), d.data("fr-position", "before"))) : "after" === d.data("fr-position") && l.is(d.data("fr-tag")) || (s = parseFloat(l.css("margin-bottom")) || 0, m(l.offset().top + p(t).height() + s / 2 - g.$box.offset().top, l.offset().left - g.win.pageXOffset - g.$box.offset().left, l.width()), d.data("fr-position", "after")), d.data("fr-tag", l), d.addClass("fr-visible"), g.$box.append(d)
                } else d && 0 < g.$box.find(d).length && d.removeClass("fr-visible")
            }(e))
        }
        ...
        function n(e) {
            ...
            d && d.hasClass("fr-visible") && g.$box.find(d).length ? a(e) : t.length && (e.preventDefault(), e.stopPropagation()), d && g.$box.find(d).length && d.removeClass("fr-visible"), t.removeClass("fr-dragging")
        }
        ...
        function a(e) {
            ...
            if (t.length) {
                if (e.preventDefault(), e.stopPropagation(), d && d.hasClass("fr-visible") && g.$box.find(d).length) d.data("fr-tag")[d.data("fr-position")]('<span class="fr-marker"></span>'), d.removeClass("fr-visible"); else if (!1 === g.markers.insertAtPoint(e.originalEvent)) return !1;
                ...
                if (r === g || g.undo.canDo() || g.undo.saveStep(), g.core.isEmpty()) g.$el.html(a); else {
                    ...
                    0 === a.find(o).length ? p(o).replaceWith(a) : 0 === t.find(o).length && p(o).replaceWith(t), t.after(c.MARKERS), g.selection.restore()
                }
            ...
            d && d.removeClass("fr-visible"), g.undo.canDo() || g.undo.saveStep(), setTimeout(function () {
                g.undo.saveStep()
            }, 0)
        }
